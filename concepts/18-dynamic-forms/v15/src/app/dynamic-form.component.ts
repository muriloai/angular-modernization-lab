...
export class DynamicFormComponent implements OnInit {
  // Catálogo de esquemas JSON para calibração de sensores de campo
  ...
  selectedTypeId = 'umidade';
  activeSchema!: SensorSchema;
  form!: FormGroup;

  dadosConfigurados: any = null;
  salvoComSucesso = false;

  ngOnInit(): void {
    this.selecionarEsquema(this.selectedTypeId);
  }

  selecionarEsquema(tipoId: string): void {
    this.selectedTypeId = tipoId;
    const encontrado = this.schemas.find(s => s.tipoId === tipoId);
    if (encontrado) {
      this.activeSchema = encontrado;
      this.construirFormulario(encontrado.fields);
    }
  }

  construirFormulario(fields: DynamicFieldConfig[]): void {
